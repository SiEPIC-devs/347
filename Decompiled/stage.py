...
"""
import pickle, MMC_100 as mmc100, serial_communication as sc, serial, time
...
class stage(sc.mmc100_communicator):
    ...
    def stage_init(self):
        ...
        """
        self.set_velocity(1, float(self.dict["motor_velocity"]))
        self.set_velocity(2, float(self.dict["motor_velocity"]))
        time.sleep(1)
        self.loop_mode(1, 3)
        self.loop_mode(2, 3)
        self.set_velocity(1, "?")
        print("Stage x velocity: {} um/s".format(self.serial_port.read_until("\n\r").decode("ascii").strip("#").strip("\n\r")))
        self.set_velocity(2, "?")
        print("Stage y velocity: {} um/s".format(self.serial_port.read_until("\n\r").decode("ascii").strip("#").strip("\n\r")))
        self.set_trigger(1, 1)
        self.set_trigger(2, 1)
    ...
    def emergency_stop(self):
        ...
        halts all axis
        """
        self.serial_write(mmc100.emergency_stop(stage.ALL_AXIS))

    def move_x_axis(self, distance):
        """
        move stage (distance) um
        """
        self.serial_write(mmc100.move_rel(stage.X_AXIS, distance * 0.001))
    ...
    def move_y_axis(self, distance):
        ...
        move stage (distance) um
        """
        self.serial_write(mmc100.move_rel(stage.Y_AXIS, distance * 0.001))
        self.update_parameter("automated_move_happened", str(1))
    ...
    def move_z_axis(self, distance):
        ...
        move stage (distance) um
        """
        self.serial_write(mmc100.move_rel(stage.Z_AXIS, distance * 0.001))
        self.update_parameter("automated_move_happened", str(1))
    ...
    def move_fr_axis(self, distance):
        ...
        """
        self.serial_write(mmc100.move_rel(stage.FR_AXIS, distance))

    def move_cr_axis(self, distance):
        """
        rotate chip
        """
        self.serial_write(mmc100.move_rel(stage.CR_AXIS, distance))
    ...
    def read_position(self):
        ...
        Note: Setting the axis to 0 will cause an error
        """
        readx = mmc100.position(stage.X_AXIS)
        ready = mmc100.position(stage.Y_AXIS)
        readz = mmc100.position(stage.Z_AXIS)
    ...
    def read_fiber_position(self):
        readangle = mmc100.position(stage.FR_AXIS)
    ...
    def set_encoder_polarity(self, normal_reverse):
        self.serial_write(mmc100.set_encoder(stage.ALL_AXIS, normal_reverse))
    ...
    def move_and_velocity_z(self, distance):
        self.serial_write(mmc100.move_rel(stage.Z_AXIS, distance * 0.001) + ";" + mmc100.encoder_velocity(stage.Z_AXIS))
        return self.serial_port.read_until("\n\r").decode("ascii")
