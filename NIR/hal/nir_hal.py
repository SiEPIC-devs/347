...
class LaserHAL(ABC):
    ...
    async def configure_sweep(self, config: SweepConfig) -> bool:
        """Configure sweep with comprehensive params, TODO: CONFIG PLACEHOLDER"""
        success = True
        success &= await self.set_sweep_range(config.range.start, config.range.stop)
        success &= await self.set_sweep_speed(config.speed)
        # Additional sweep configuration can be implemented by subclasses
        # 
        return success
