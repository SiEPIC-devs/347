...
class FineAlign:
    ...
    async def spiral_search(self, x_pos: float, y_pos: float) -> bool:
        ...
        try:
            ...
            self.log(f"Starting spiral search from ({x_pos:.2f}, {y_pos:.2f}) with detector: {self.primary_detector}", "info")
            ...
            while num_steps < limit and not self._stop_requested:
                ...
                for _ in range(num_steps):
                    ...
                    if current_loss > best["loss"]:
                        ...
                        self.log(f"New best position: ({x_pos:.2f}, {y_pos:.2f}) with power: {current_loss:.2f}dBm", "debug")
            ...
            # Go to best position
            self.log(f"Moving to best position: ({best['position'][0]:.2f}, {best['position'][1]:.2f}) with power: {best['loss']:.2f}dBm", "info")
    ...
    async def gradient_search(self):
        ...
        async def _move_dir(axis, dir):
            ...
            if loss > self.lowest_loss:
                ...
                if axis == AxisType.X:
                    self.best_position[0] = self.stage_manager.get_position(axis) + dir*self.step_size
                else:  # AxisType.Y
                    self.best_position[1] = self.stage_manager.get_position(axis) + dir*self.step_size
                return axis, dir
            else:
                return False, False
        try:
            ...
            x = await self.stage_manager.read_position(AxisType.X)
            y = await self.stage_manager.read_position(AxisType.Y)
